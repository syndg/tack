...
Items intentionally deferred from earlier implementation plans or parked for later design. Each entry tracks where it was originally scoped, why it was deferred, and when it should land.
...
- **Original scope:** Sandbox provider interface + Daytona implementation
...
- **Original scope:** Agent runtime interface + Pi implementation
...
- **Details:** Several `internal/config/config.go` defaults diverge from earlier implementation notes:
...
- **Deferred to:** Align defaults to match actual implementations after more runtime usage
...
- **Deferred to:** Ship default config when real providers are configured
...
- **Deferred to:** Wire planner spawning to the persisted mode
- **Reason:** Planning now records the intent correctly, but there is still no runtime planner execution loop to act on it.
...
- **Original scope:** Nested sub-execution escalation on stream failure
...
- **Deferred to:** Future execution hardening work that defines delivery semantics
...
- **Original scope:** Tiered merge strategy (clean → auto-resolve → AI)
...
- **Deferred to:** Future merge hardening work that defines LLM integration
...
- **Deferred to:** Future execution hardening work that defines watchdog behavior
