...
This example shows a Pi runtime with local sandboxes — the best-supported and recommended configuration.
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `runtime` | string | `"pi"` | Agent runtime. `pi` is the recommended first-class option; `claude-code` remains available with minimal support. See [Agent Runtimes](/docs/getting-started/agent-runtimes). |
...
| `pi.provider` | string | `"anthropic"` | LLM provider for the Pi runtime. The recommended setup may override this to `openai-codex`. |
...
List of shell commands. Each command runs in the sandbox. Non-zero exit means failure. The built-in default is the Go-oriented set shown below; many projects override it in `.tack/config.yaml`. Common configurations:
