...
```mermaid
flowchart TB
  objective[Objective] --> discovery[Discovery dossier]
  discovery --> planner[Planner creates stream plan]
  planner --> approval[Human approval]
  approval --> dispatch[Dispatch parallel streams]

  subgraph streams[Isolated stream execution]
    builder[Builder agents]
    gates[Quality gates]
    reviewer[Reviewer agents]
    recovery[Recovery loop]
    builder --> gates --> reviewer
    reviewer -->|needs changes| recovery --> builder
  end

  dispatch --> builder
  reviewer --> merge[Merge queue]
