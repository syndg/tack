...
Live stream of agent activity — tool calls, file edits, test results, recovery attempts, and status changes.
...
If a stream cannot heal automatically, the objective can become `partial` or `blocked` instead of disappearing into a failed run. That preserves the stream state and recovery context for a later retry.
