...
```mermaid
flowchart TB
  base[Base branch] --> sandboxA[Stream A sandbox]
  base --> sandboxB[Stream B sandbox]
  base --> sandboxC[Stream C sandbox]

  sandboxA --> branchA[Stream A branch]
  sandboxB --> branchB[Stream B branch]
  sandboxC --> branchC[Stream C branch]

  branchA --> merge[Merge queue]
  branchB --> merge
  branchC --> merge
  merge --> integrated[Integrated result]
```
