...
```mermaid
flowchart TB
  entry[Merge entry ready] --> deps{Dependencies merged?}
  deps -->|no| wait[Wait in queue]
  wait --> deps
  deps -->|yes| clean[Try clean merge]
  clean --> cleanResult{Clean?}
  cleanResult -->|yes| gates[Post-merge gates]
  cleanResult -->|no| auto[Try auto-resolve]
  auto --> autoResult{Resolved?}
  autoResult -->|yes| gates
  autoResult -->|no| human[Human escalation]
  gates --> passed{Gates pass?}
  passed -->|yes| merged[Record merged]
  passed -->|no| revert[Revert and mark failed]
```
