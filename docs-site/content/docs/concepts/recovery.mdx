...
```mermaid
flowchart TB
  fail[Step fails] --> ledger[Record failure in recovery ledger]
  ledger --> classify[Classify failure kind]
  classify --> policy[Apply retry policy]
  policy --> budget{Retries remain?}
  budget -->|yes| automatic[Automatic recovery]
  automatic --> rerun[Rerun step or builder]
  rerun --> check{Recovered?}
  check -->|yes| continue[Continue workflow]
  check -->|no| ledger
  budget -->|no| blocked[Block for human guidance]
  blocked --> guidance[Operator sends guidance]
  guidance --> automatic
```
