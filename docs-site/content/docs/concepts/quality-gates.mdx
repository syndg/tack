...
Quality gates run against the **full project**, but Deck only fails a stream for errors in **its own files**.
...
When gates fail for errors in the stream's file scope, Deck sends the errors back to the builder agent with a "fix this" instruction. The agent iterates until gates pass or the max iteration limit is reached.
