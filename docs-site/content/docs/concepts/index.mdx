...
|------|-------------|-------------|
| **Planner** | Explores the codebase, decomposes the objective into parallel streams with file scopes and dependencies | First, after objective is created |
| **Scout** | Optional pre-build research inside a bounded scope — investigates files or patterns before building | Before builder, if the blueprint includes it |
| **Builder** | Implements the stream's task in an isolated sandbox, commits changes | After scout (if present) or directly |
