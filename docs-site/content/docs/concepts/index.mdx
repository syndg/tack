...
```mermaid
flowchart TB
  objective[Objective] --> discovery[Discovery]
  discovery --> dossier[Context dossier]
  dossier --> planner[Planner]
  planner --> plan[Plan: streams, scopes, dependencies]
  plan --> approval[Human approval]
  approval --> dispatch[Dispatch]

  dispatch --> streamA[Stream A]
  dispatch --> streamB[Stream B]
  dispatch --> streamC[Stream C]

  streamA --> gatesA[Quality gates]
  streamB --> gatesB[Quality gates]
  streamC --> gatesC[Quality gates]

  gatesA --> reviewA[Review]
  gatesB --> reviewB[Review]
  gatesC --> reviewC[Review]

  reviewA --> merge[Merge queue]
  reviewB --> merge
  reviewC --> merge

  merge --> postMerge[Post-merge gates]
  postMerge --> pr[Pull request]
  pr --> complete[Objective complete]
```
...
|------|-------------|-------------|
| **Discovery** | Explores the codebase, produces a cited context dossier | First, before planning — see [Discovery & Dossiers](/docs/concepts/discovery) |
...
The merge processor is a deterministic step, not an agent. It integrates ready streams and runs post-merge checks. See [Merge Queue](/docs/concepts/merge-queue).
